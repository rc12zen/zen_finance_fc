							...
							<div>
								...
								<p className="text-[11px] text-gray-300 mt-1">
									...
									<span className="text-emerald-400 font-bold">
										{runCompletionSummary.matched.toLocaleString()} found
									</span>
									,{" "}
									<span className="text-red-400 font-bold">
										{runCompletionSummary.notMatched.toLocaleString()} not found
									</span>
									,{" "}
									<span className="text-amber-400 font-bold">
										{runCompletionSummary.pendingReview.toLocaleString()}{" "}
										pending review
									</span>
									.
								</p>
					...
					<div className="bg-white border border-gray-200 p-5 shadow-xs flex flex-col justify-between min-h-[180px]">
						<div>
							...
							>
								...
								<span>
									{agingUploading ? "Uploading…" : "Upload From Local"}
								</span>
							</button>
						</div>
					...
					<div className="bg-white border border-gray-200 p-5 shadow-xs flex flex-col justify-between min-h-[180px]">
						<div>
							...
							>
								<UploadCloud size={14} className="text-[#4A90E2]" />
								<span>
									{statementUploading ? "Uploading…" : "Upload From Local"}
								</span>
							</button>
						</div>
						{files.length > 0 ? (
							<div className="mt-3 pt-2 border-t border-gray-100 space-y-1.5 max-h-[120px] overflow-y-auto">
								{files.map((f) => (
									<div
										key={f.filename}
										className="flex items-center justify-between text-[11px] bg-gray-50 border border-gray-200 rounded-xs px-2 py-1.5 gap-2"
									>
										<div className="flex items-center gap-1.5 min-w-0">
											<FileText size={11} className="text-gray-400 shrink-0" />
											<span className="font-mono font-bold text-primary truncate text-[10px]">
												{f.filename}
											</span>
											<span className="text-gray-400 shrink-0 text-[10px]">
												{f.bank_name} · {f.size_mb}MB
											</span>
										</div>
										<button
			...
			isRunning || loading
				...
				>
						...
						<div className="text-xs font-medium">
							{isRunning ? (
								<span className="font-bold text-white">
									Running…{" "}
									{runStatus.progress_current > 0
										? `${runStatus.progress_current} rows processed`
										: ""}
								</span>
							) : agingStatus.loaded && files.length > 0 ? (
					...
					<button
						...
						className={`w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 font-bold text-xs uppercase tracking-widest transition-all disabled:opacity-20 disabled:cursor-not-allowed shadow-xs whitespace-nowrap cursor-pointer
      ${
				!(isRunning || loading) && agingStatus.loaded && files.length > 0
				...
				<div className="bg-white border border-gray-200 p-6 shadow-xs space-y-6">
					...
					<div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-1">
						{[
							{
								icon: <Layers size={13} className="text-[#1E3A5F]" />,
								label: "Total rows ingested",
								value: dm.total_rows_ingested ?? 0,
								sub: "Transactions read",
							},
							{
								icon: <Sparkles size={13} />,
								label: "Found",
								value: dm.found ?? 0,
								sub: "Invoice found in aging",
							},
							{
								icon: <AlertTriangle size={13} className="text-[#2E6DA4]" />,
								label: "Not Found",
								value: dm.not_found ?? 0,
								sub: "Require manual review",
							},
							{
								icon: <ShieldCheck size={13} className="text-[#4A90E2]" />,
								label: "Passed Validation",
								value: dm.passed_validation ?? 0,
								sub: "All checks passed",
							},
							{
								icon: <Ban size={13} className="text-[#e11d48]" />,
								label: "Failed Validation",
								value: dm.failed_validation ?? 0,
								sub: "At least one check failed",
							},
							{
								icon: <Calendar size={13} className="text-[#f59e0b]" />,
								label: "Pending Approval",
								value: dm.pending_hitl ?? 0,
								sub: "Awaiting SPOC review",
							},
							{
								icon: <ClipboardCheck size={13} className="text-emerald-600" />,
								label: "Approved & Posted",
								value: dm.posted_to_oracle ?? 0,
								sub: "Posted to Oracle Fusion",
							},
							{
								icon: <Ban size={13} className="text-red-500" />,
								label: "Rejected",
								value: dm.rejected ?? 0,
								sub: "Rejected by SPOC",
							},
						].map(({ icon, label, value, sub }) => (
							<div
								key={label}
								className="border border-gray-200 p-4 rounded-sm bg-gray-50/30"
							>
									...
									const val =
										key === "found"
											...
											: key === "notFound"
												? (dm.not_found ?? 0)
												: key === "passed"
													? (dm.passed_validation ?? 0)
													: key === "failed"
														? (dm.failed_validation ?? 0)
														: (dm.pending_hitl ?? 0);
									return (
										<button
											key={key}
